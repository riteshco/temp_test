use crate::config::{init,Config, get_log_target, set_log_target};
            ...
            if let Some(group_name) = group_path.file_name().and_then(|n| n.to_str()) {
                for hash_entry in fs::read_dir(&group_path)? {
                    ...
                    if hash_entry.file_type()?.is_file() {
                        if let Some(hash) = hash_entry.file_name().to_str() {
                            let name_path = cache_base_path.join("names").join(hash);
