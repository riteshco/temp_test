...
pub async fn fetch_and_decode_file_from_github(
    ...
    let url = format!(
        ...
        config.github_owner, config.github_repo, path, commit_ref
    );
...
pub fn update_local_cache(
    ...
    let cache_file_path = if project.is_empty() && first_child_directory == "names" {
        cache_base_path.join("names").join(hash)
    } else {
        cache_base_path
            .join("access")
            .join(first_child_directory) // server name
            .join(project) // group name
            .join(hash)
    };
...
pub async fn sync_full_cache(config: &Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tree_url = format!(
        ...
        config.github_owner, config.github_repo, config.branch
    );
